...
package object nodescala {
  ...
  implicit class FutureCompanionOps(val f: Future.type) extends AnyVal {
    ...
    def any[T](fs: List[Future[T]]): Future[T] = {
      @tailrec
      def waitForFirst(fs: List[Future[T]]): Future[T] = {
        fs.find(_.isCompleted) match {
          case Some(f) => f
          case _ => waitForFirst(fs)
        }
      }
      waitForFirst(fs)
    }
  ...
  implicit class FutureOps[T](val f: Future[T]) extends AnyVal {
     ...
     */
    def continueWith[S](cont: Future[T] => S): Future[S] = Try {
      for {
        _ <- f
      } yield cont(f)
    } match {
      case Success(x) => x
      case Failure(_) => Future(cont(f))
    }
