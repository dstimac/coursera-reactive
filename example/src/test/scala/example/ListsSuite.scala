...
class ListsSuite extends FunSuite {
  ...
  test("fetch head throws exception if list is empty") {
    intercept[NoSuchElementException] {
      fetchHead(List.empty)
    }
  }

  def fetchHead(list: List[Int]): Int = {
    if(list.isEmpty) throw new scala.NoSuchElementException("empty list")
    else list.head
  }
